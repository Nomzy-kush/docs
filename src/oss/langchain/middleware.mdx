...
1. **Global limit** - Applies to all tool calls (when `tool_name` is not specified)
2. **Tool-specific limit** - Applies only to a specific tool (when `tool_name` is specified)

You can use both in the same agent to enforce different limits for different tools.

#### Limit types

For each limit scope (global or tool-specific), you can specify two types of limits:

1. **Thread limit** (`thread_limit`) - Maximum tool calls across all runs within a thread (conversation)
   - Persists across multiple agent invocations with the same thread ID
   - Useful for enforcing daily/session quotas
   - Requires a checkpointer to track state across runs

2. **Run limit** (`run_limit`) - Maximum tool calls in a single agent invocation
   - Resets with each new user message
   - Useful for preventing loops in a single turn
   - Works without a checkpointer

You must specify at least one limit type (thread or run), but can specify both.

#### Exit behaviors

When a limit is reached, you can choose how the agent should respond:

1. **`"continue"`** (default) - Most flexible approach
   - Blocks only the exceeded tool calls with error messages
   - Other tools and the agent continue executing normally
   - The model sees the error messages and can adapt its strategy
   - Best for: Most use cases where you want the agent to handle limits gracefully

2. **`"error"`** - Strict enforcement
   - Raises `ToolCallLimitExceededError` exception immediately
   - Stops all execution when any limit is exceeded
   - Best for: Critical limits that must never be exceeded

3. **`"end"`** - Immediate termination
   - Stops execution with ToolMessage + AI message for the exceeded call
   - Only works when limiting a specific tool with no other pending tool calls
   - Raises `NotImplementedError` if other tools have pending calls
   - Best for: Single-tool scenarios where you want clean termination
...
# Example 1: Global limit across all tools
# - Max 20 tool calls total per thread (across multiple runs)
# - Max 10 tool calls total per single run
# - Default "continue" behavior: blocked calls return errors, agent keeps going
global_limiter = ToolCallLimitMiddleware(
    thread_limit=20,
    run_limit=10,
)

# Example 2: Specific tool limit with "continue" behavior
# - Max 5 calls to "search" tool per thread
# - Max 3 calls to "search" tool per run
# - Blocked search calls return errors, but other tools can still be called
search_limiter = ToolCallLimitMiddleware(
    tool_name="search",
    thread_limit=5,
    run_limit=3,
    exit_behavior="continue",  # Explicit, but this is the default
)

# Example 3: Thread limit only (no per-run limit)
database_limiter = ToolCallLimitMiddleware(
    tool_name="query_database",
    thread_limit=10,  # Max 10 database queries per conversation
)

# Example 4: Run limit with "error" behavior
# - Raises exception immediately when limit is exceeded
# - Useful for critical operations that must be strictly limited
web_scraper_limiter = ToolCallLimitMiddleware(
    tool_name="scrape_webpage",
    run_limit=2,  # Max 2 web scrapes per turn
    exit_behavior="error",  # Strict enforcement
)

# Example 5: "end" behavior (single tool only)
# - Stops immediately when limit is exceeded
# - Only works when this is the only tool being called
# - Raises NotImplementedError if other tools have pending calls
critical_tool_limiter = ToolCallLimitMiddleware(
    tool_name="delete_records",
    run_limit=1,
    exit_behavior="end",  # Stop immediately after one call
)
...
agent = create_agent(
    ...
    middleware=[global_limiter, search_limiter, database_limiter, web_scraper_limiter],
)
```
:::
...
const searchLimiter = toolCallLimitMiddleware({
  ...
  threadLimit: 5,
  runLimit: 3,
  exitBehavior: "continue", // Explicit, but this is the default
});

// Example 3: Thread limit only (no per-run limit)
const databaseLimiter = toolCallLimitMiddleware({
  toolName: "query_database",
  threadLimit: 10, // Max 10 database queries per conversation
});

// Example 4: Run limit with "error" behavior
// - Throws exception immediately when limit is exceeded
// - Useful for critical operations that must be strictly limited
const webScraperLimiter = toolCallLimitMiddleware({
  toolName: "scrape_webpage",
  runLimit: 2, // Max 2 web scrapes per turn
  exitBehavior: "error", // Strict enforcement
});

// Example 5: "end" behavior (single tool only)
// - Stops immediately when limit is exceeded
// - Only works when this is the only tool being called
// - Throws error if other tools have pending calls
const criticalToolLimiter = toolCallLimitMiddleware({
  toolName: "delete_records",
  runLimit: 1,
  exitBehavior: "end", // Stop immediately after one call
});
