...
This guide shows how to read and process [experiment](/langsmith/evaluation-concepts#experiment) results directly from the [`Client.evaluate()`](https://reference.langchain.com/python/langsmith/observability/sdk/client/#langsmith.client.Client.evaluate) response.
...
The [`evaluate()`](https://reference.langchain.com/python/langsmith/observability/sdk/client/#langsmith.client.Client.evaluate) function returns an iterator when called with `blocking=False`. This allows you to process results as they're produced:
